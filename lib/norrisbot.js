...
NorrisBot.prototype._codewarsSetup = function(message) {
    ...
    console.log(message.text.toLowerCase().split("codewars setup ")[1])
  ...
  if (!/javascript|ruby/.test(argv.language)){
    console.log("Setup failed, unsupported language: " + argv.language.toString());
   process.exit(1);
   }
  ...
  self.cwars.setup({
    ...
    console.log("All done. Settings saved in ~/.config/codewars/settings.json".green);
  }).catch(function(err) {
    console.log(err);
  }) 
